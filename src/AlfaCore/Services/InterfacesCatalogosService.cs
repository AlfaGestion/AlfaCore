    ...
    ICompanyBrandingService companyBrandingService) : IInterfacesCatalogosService
{
    ...
    private async Task<CatalogosCatalogoDetalleDto?> GetCatalogoInternalAsync(int idInsert, bool soloPublico, CancellationToken ct, int? expectedBaseId = null)
        ...
        {
            var connectionString = ResolveConnectionString(expectedBaseId, soloPublico ? "Catalogos.GetCatalogoPublico" : "Catalogos.GetCatalogo");
            await using var cn = new SqlConnection(connectionString);
            await cn.OpenAsync(token);
            var availability = await GetCatalogosAvailabilityAsync(token);
    ...
    private async Task<CatalogosSourceAvailability> GetCatalogosAvailabilityAsync(CancellationToken ct, int? expectedBaseId = null)
    {
        var connectionString = ResolveConnectionString(expectedBaseId, "Catalogos.GetCatalogosAvailability");
