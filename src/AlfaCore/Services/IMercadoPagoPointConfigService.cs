...
{
    Task<MercadoPagoPointConfigDto> GetConfiguracionAsync(CancellationToken ct = default);
    ...
    Task<MercadoPagoPointOptions?> ResolveOptionsForPuntoVentaAsync(int idPuntoVenta, CancellationToken ct = default);
    ...
    /// <summary>Devuelve los códigos de medio configurados para Point. Incluye compatibilidad con
    /// la clave histórica MERCADOPAGO_CODIGO_MEDIO_PAGO.</summary>
    Task<IReadOnlySet<string>> ResolveMediosConfiguradosAsync(CancellationToken ct = default);

    Task GuardarConfiguracionAsync(MercadoPagoPointConfigDto configuracion, CancellationToken ct = default);
