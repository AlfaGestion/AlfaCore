...
public interface ICarritoComprasService
{
    ...
    Task<PagedResult<CarritoComprasResumenDto>> SearchAsync(CarritoComprasFiltroDto filtro, int? expectedBaseId, CancellationToken ct = default);
    Task<CarritoComprasGeneralDetalleDto?> GetGeneralAsync(int idCarrito, CancellationToken ct = default);
    Task<CarritoComprasGeneralDetalleDto?> GetGeneralAsync(int idCarrito, int? expectedBaseId, CancellationToken ct = default);
    ...
    Task<CatalogosCatalogoDetalleDto?> GetPublicCartAsync(int idInsert, string? idWeb, string? codigoCliente, int? expectedBaseId, CancellationToken ct = default);
