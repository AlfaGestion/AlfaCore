    ...
    ILogger<CargaViajesService> logger) : ICargaViajesService
{
    ...
    public Task<PagedResult<CargaViajesGridItemDto>> SearchViajesAsync(CargaViajesFilters filters, CancellationToken ct = default)
        => ExecuteLoggedAsync(ModuleName, "SearchViajes", async token =>
        {
            ...
            if (!await TableExistsAsync(cn, viajeTable, token))
                return new PagedResult<CargaViajesGridItemDto> { Items = [], Total = 0, PageNumber = pageNumber, PageSize = pageSize };
            ...
            var idComprobanteColumn = FirstExistingColumnOrNull(columns, "IDCOMPROBANTE", "COMPROBANTE");
            var totalImporteColumn = FirstExistingColumnOrNull(columns, "TOTAL_IMPORTE", "TOTALIMPORTE");
            var totalFleteColumn = FirstExistingColumnOrNull(columns, "TOTAL_FLETE", "TOTALFLETE");
            var estadoColumn = FirstExistingColumnOrNull(columns, "ESTADO");
            var usuarioColumn = FirstExistingColumnOrNull(columns, "USUARIO");
            var altaColumn = FirstExistingColumnOrNull(columns, "FECHAHORA_ALTA", "FECHAHORAALTA", "FECHAHORA_GRABACION");
            var clienteCodeExpr = SqlStringColumnOrEmpty("v", clienteColumn);
            var destinoCodeExpr = SqlStringColumnOrEmpty("v", destinoColumn);
            var choferCodeExpr = SqlStringColumnOrEmpty("v", choferColumn);
            var tipoVehiculoCodeExpr = SqlStringColumnOrEmpty("v", tipoVehiculoColumn);
            var idComprobanteExpr = SqlStringColumnOrEmpty("v", idComprobanteColumn);
            var totalImporteExpr = SqlDecimalColumnOrZero("v", totalImporteColumn);
            var totalFleteExpr = SqlDecimalColumnOrZero("v", totalFleteColumn);
            var estadoExpr = string.IsNullOrWhiteSpace(estadoColumn) ? "N'PENDIENTE'" : $"ISNULL(v.{estadoColumn}, N'PENDIENTE')";
            var usuarioExpr = SqlStringColumnOrEmpty("v", usuarioColumn);
            var altaExpr = string.IsNullOrWhiteSpace(altaColumn) ? "CAST(NULL AS datetime)" : $"v.{altaColumn}";
            var hasDestinoTable = await TableExistsAsync(cn, "TA_DESTINOS", token);
            var hasChoferTable = await TableExistsAsync(cn, "TA_CHOFERES", token);
            var hasVehiculoTable = await TableExistsAsync(cn, "TA_TIPOVEHICULO", token);
            var clienteJoin = $"LEFT JOIN dbo.Vt_Clientes cli ON UPPER(LTRIM(RTRIM(ISNULL(cli.CODIGO, '')))) = UPPER(LTRIM(RTRIM({clienteCodeExpr})))";
            var destinoJoin = hasDestinoTable ? $"LEFT JOIN dbo.TA_DESTINOS d ON UPPER(LTRIM(RTRIM(ISNULL(d.CODIGO, '')))) = UPPER(LTRIM(RTRIM({destinoCodeExpr})))" : string.Empty;
            var choferJoin = hasChoferTable ? $"LEFT JOIN dbo.TA_CHOFERES ch ON UPPER(LTRIM(RTRIM(ISNULL(ch.CODIGO, '')))) = UPPER(LTRIM(RTRIM({choferCodeExpr})))" : string.Empty;
            var vehiculoJoin = hasVehiculoTable ? $"LEFT JOIN dbo.TA_TIPOVEHICULO tv ON UPPER(LTRIM(RTRIM(ISNULL(tv.CODIGO, '')))) = UPPER(LTRIM(RTRIM({tipoVehiculoCodeExpr})))" : string.Empty;
            var destinoDescExpr = hasDestinoTable ? $"ISNULL(d.DESCRIPCION, {SqlStringColumnOrEmpty("v", FirstExistingColumnOrNull(columns, "DESCRIPCIONDESTINO"))})" : SqlStringColumnOrEmpty("v", FirstExistingColumnOrNull(columns, "DESCRIPCIONDESTINO"));
            var choferNameExpr = hasChoferTable ? $"ISNULL(ch.NOMBRES, {SqlStringColumnOrEmpty("v", FirstExistingColumnOrNull(columns, "NOMBRE_CHOFER", "NOMBRECHOFER"))})" : SqlStringColumnOrEmpty("v", FirstExistingColumnOrNull(columns, "NOMBRE_CHOFER", "NOMBRECHOFER"));
            var vehiculoExpr = hasVehiculoTable ? "ISNULL(tv.DESCRIPCION, '')" : "CAST('' AS nvarchar(100))";
            ...
            var sql = $"""
                SELECT
                    ...
                    {idComprobanteExpr} AS IdComprobante,
                    v.FECHA AS Fecha,
                    ...
                    {totalImporteExpr} AS TotalCliente,
                ...
                WHERE (
                  ...
                  AND (@Estado = '' OR UPPER(LTRIM(RTRIM({estadoExpr}))) = @Estado)
                  AND (@Usuario = '' OR UPPER(LTRIM(RTRIM({usuarioExpr}))) = @Usuario)
                  AND (@IdComprobante = '' OR {idComprobanteExpr} = @IdComprobante)
            ...
            var sql = $"""
                ;WITH base_rows AS (
                    SELECT
                        ...
                        {tarifaFleteroExpr} AS TarifaFletero,
                        {SqlDecimalColumnOrZero("t", FirstExistingColumnOrNull(columns, "PorcentajeAdic"))} AS PorcentajeAdic,
                        {SqlDecimalColumnOrZero("t", FirstExistingColumnOrNull(columns, "PorcentajeAdic1"))} AS PorcentajeAdic1,
    ...
    public Task ActualizarAdicionalesTarifasClienteAsync(string clienteCodigo, IReadOnlyCollection<string> listasSeleccionadas, CargaViajeTarifaSaveRequest source, CancellationToken ct = default)
        => ExecuteLoggedAsync(ModuleName, "ActualizarAdicionalesTarifasCliente", async token =>
        {
            ...
            if (string.IsNullOrWhiteSpace(cliente))
                throw new InvalidOperationException("No se recibiÃ³ el cliente para actualizar los adicionales.");
    ...
    public Task<PagedResult<CargaViajeDestinoGridItemDto>> SearchDestinosAsync(CargaViajesFilters filters, CancellationToken ct = default)
        => ExecuteLoggedAsync(ModuleName, "SearchDestinos", async token =>
        {
            ...
            if (!await TableExistsAsync(cn, table, token))
                return new PagedResult<CargaViajeDestinoGridItemDto> { Items = [], Total = 0, PageNumber = pageNumber, PageSize = pageSize };
            ...
            var activoSelect = hasActivo ? "CAST(ISNULL(Activo, 1) AS bit)" : "CAST(1 AS bit)";
            var textoLike = SearchTextHelper.LikeContains(filters.Texto);
            var activoFiltro = ParseNullableBitFilter(filters.Activo);
            var orderBy = BuildOrderByClause(
                filters.SortBy,
    ...
    public Task<CargaViajeLookupOptionDto> CreateTipoVehiculoRapidoAsync(string descripcion, CancellationToken ct = default)
        => ExecuteLoggedAsync(ModuleName, "CreateTipoVehiculoRapido", async token =>
        {
            ...
            var validation = new ValidationResult();
            if (string.IsNullOrWhiteSpace(normalized))
                validation.Add("Descripcion", "Debe ingresar un tipo de vehÃ­culo.");
            ...
            var maxLength = await GetColumnCharacterLengthAsync(cn, table, "DESCRIPCION", token, 100);
            if (maxLength > 0 && normalized.Length > maxLength)
                validation.Add("Descripcion", $"El tipo de vehÃ­culo no puede superar {maxLength} caracteres.");
            ...
            if (affected <= 0)
                throw new InvalidOperationException("No se pudo crear el tipo de vehÃ­culo.");

            logger.LogInformation("CreateTipoVehiculoRapido OK Codigo={Codigo} Descripcion={Descripcion}", codigo, normalized);
            return new CargaViajeLookupOptionDto { Codigo = codigo, Titulo = normalized };
        }, "No se pudo crear el tipo de vehÃ­culo.", ct);
    ...
    public Task<CargaViajeTarifaGridItemDto?> ClonarTarifaParaViajeAsync(string idListaBase, CargaViajeSaveRequest viaje, CancellationToken ct = default)
        => ExecuteLoggedAsync(ModuleName, "ClonarTarifaParaViaje", async token =>
        {
            ...
            if (string.IsNullOrWhiteSpace(clienteCodigo) || string.IsNullOrWhiteSpace(choferCodigo) || string.IsNullOrWhiteSpace(destinoCodigo) || string.IsNullOrWhiteSpace(tipoVehiculoCodigo))
                throw new InvalidOperationException("Debe completar cliente, chofer, destino y tipo vehÃ­culo antes de duplicar la tarifa.");
            ...
            if (source is null)
                throw new InvalidOperationException("No se encontrÃ³ la tarifa base seleccionada.");
    ...
    public Task<IReadOnlyList<CargaViajeReporteLiquidacionRowDto>> SearchLiquidacionChoferesAsync(CargaViajesReporteLiquidacionFilters filters, CancellationToken ct = default)
        ...
        }, "No se pudo generar la liquidaciÃ³n de choferes y fleteros.", ct);
    ...
    public async Task<CargaViajesLookupDto> GetLookupsAsync(CancellationToken ct = default)
    {
        ...
        return await ExecuteLoggedAsync(ModuleName, "GetLookups", async token =>
        {
            ...
            result.TipoVehiculos = await LoadOptionalLookupAsync("tipos de vehÃ­culo", t => SearchTipoVehiculosLookupAsync(string.Empty, t), token);
            return result;
        ...
        CancellationToken ct)
    {
        ...
        {
            logger.LogWarning(ex, "No se pudo cargar la lista auxiliar {LookupName} de carga de viajes. Se continÃºa con la lista vacÃ­a.", lookupName);
            return [];
        }
    }
    ...
    public Task<string> GetNextIdComprobanteAsync(CancellationToken ct = default)
        => ExecuteLoggedAsync(ModuleName, "GetNextIdComprobante", async token =>
        {
            ...
            if (sucursal.Length != 4 || letra.Length != 1)
                throw new InvalidOperationException("VIAJES-SUCURSAL debe tener 4 caracteres y VIAJES-LETRA debe tener 1 carÃ¡cter.");
    ...
    private static async Task<string> GetNextCodigoDisponibleAsync(SqlConnection cn, string table, string column, CancellationToken ct)
    {
        ...
        while (await ExistsByCodeAsync(cn, table, column, next, ct))
        {
            attempts++;
            if (attempts > 10000)
                throw new InvalidOperationException($"No se pudo calcular un cÃ³digo libre para {table}.");
    ...
    private Task<string> GetNextCodigoFromTableAsync(string table, string column, CancellationToken ct = default)
        => ExecuteLoggedAsync(ModuleName, $"GetNextCodigo_{table}", async token =>
        {
            ...
            return formatted;
        }, $"No se pudo obtener la prÃ³xima numeraciÃ³n de {table}.", ct);
    ...
    public Task EnsureViajesSchemaAsync(CancellationToken ct = default)
        => ExecuteLoggedAsync(ModuleName, "EnsureViajesSchema", async token =>
        {
            ...
            if (!hasViajesTable)
                logger.LogWarning("EnsureViajesSchema: no existe MV_VIAJES_CARGA en la base activa. Se omite la validaciÃ³n/actualizaciÃ³n de esa vista.");

            if (!await TableExistsAsync(cn, "TA_TARIFA", token))
            {
                logger.LogWarning("EnsureViajesSchema: no existe TA_TARIFA en la base activa. Se omite la actualizaciÃ³n de esa tabla.");
                return;
            }
            ...
            catch
            {
                ...
                {
                    logger.LogWarning(rollbackEx, "EnsureViajesSchema rollback fallÃ³");
                }
            ...
            {
                logger.LogInformation("EnsureViajesSchema sin cambios. Las tablas del mÃ³dulo Viajes ya estaban actualizadas.");
                return;
            }

            logger.LogInformation("EnsureViajesSchema creÃ³ columnas: {Columns}", string.Join(", ", createdColumns));
        }, "No se pudo verificar la estructura del mÃ³dulo Viajes.", ct);
    ...
    private static string BuildValidationMessage(ValidationResult validation)
    {
        ...
        return issues.Count == 0
            ? "RevisÃ¡ los campos marcados antes de guardar."
        ...
        CargaViajesConfigDto config)
    {
        ...
        for (var i = 0; i < AdicionalesGeneralesCount; i++)
        {
            ...
            // Cuando el adicional estÃ¡ configurado como Importe (no Porcentaje), el
        ...
        for (var i = 0; i < 3; i++)
        {
            ...
            result.ConceptosCliente.Add(BuildConcepto(string.IsNullOrWhiteSpace(descripcion) ? "Adicional fijo" : descripcion, totalAdicionalFijo, cantidadFijo, null));
        }
    ...
    private static CargaViajesConfigDto NormalizeConfiguracion(CargaViajesConfigDto config)
    {
        ...
        // histÃ³rico de configuraciones ya guardadas antes de este campo.
    ...
    private async Task PersistConfiguracionAsync(SqlConnection cn, CargaViajesConfigDto config, CancellationToken ct, bool migrateOnly)
    {
        ...
        catch
        {
            ...
            {
                logger.LogWarning(rollbackEx, "PersistConfiguracion rollback fallÃ³");
            }
        ...
        if (migrateOnly)
            logger.LogInformation("PersistConfiguracion migrÃ³ claves nuevas del mÃ³dulo de viajes.");
    }
    ...
    private static async Task UpdateAdicionalTipoAsync(SqlConnection cn, SqlTransaction tx, string detailColumn, string key, string tipo, CancellationToken ct)
    {
        ...
        // en esta misma transacciÃ³n, asÃ­ que alcanza con un UPDATE directo sobre
